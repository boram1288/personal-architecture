...
| FR-05 | 데이터 격리 영역 내 유지 | 제외 | 데이터 흐름 규칙으로, QA-01(기밀성)을 충족하는 격리 구조 위에서 달성됨. 독자적 구조 결정 없음 |
...
| FR-09 | pVM 상태 모니터링·장애 처리 | 제외 | FR-01의 생명주기 관리 구조 안에서 구현 가능. 독자적 구조 결정 없음 |

**선정 결과: FR-01, FR-02, FR-03, FR-04, FR-06, FR-07, FR-08 (7건)**
...
|----|---------------------|:--------:|:----:|----------|
| QA-01 | 보안 — Host 침해 기밀성 (QS-01) | 1순위 | **선정** | 신뢰 경계를 결정하는 최상위 driver. 신뢰 주체는 EL2(pKVM)뿐이며 Host 측 프레임워크도 비신뢰 영역으로 설계 |
...
| QA-06 | 자원 효율 (QS-06) | 3순위 | 제외 | 구조 결정 후 설계 단계에서 메모리 예약·회수 정책으로 대응 |
| QA-07 | 확장성 — 워크로드 수용 (QS-07) | 1순위 | **선정** | 과제 핵심 키워드. 플러그인 구조와 인터페이스 계약의 안정성을 강제 |
| QA-08 | 변경 용이성 — Secure OS 교체 (QS-08) | 2순위 | **선정** | Secure OS 추상화 인터페이스(교체 시 무관 SW 무수정)를 강제 |
...
| QA-11 | 성능 — HW IP 공유 오버헤드 (QS-11) | 1순위 | **선정** | 단일 컨텍스트 HW의 SW 중재에서 보안·성능이 동시에 걸리는 본 과제 고유의 난제 |

**선정 결과: QA-01, QA-02, QA-03, QA-04, QA-05, QA-07, QA-08, QA-11 (8건)**
...
| CONST-05 | 타겟 HW (단일 컨텍스트 HW IP, HW 변경 불가) | HW IP 공유를 HW 가상화 없이 SW 중재로만 실현해야 함. FR-03·QA-11 해법의 선택지를 한정 |
| CONST-06 | Secure OS 신규 개발 제외 | 기존 Secure OS의 이식·수정을 전제로 한 게스트 실행 환경(FR-07) 설계 강제 |

---

## 5. Architectural Driver 종합

선정된 driver는 총 **21건** (FR 7 + QA 8 + CONST 6)이다.

| 구분 | 선정 Driver | 제외 |
|------|------------|------|
| 기능 (FR) | FR-01, FR-02, FR-03, FR-04, FR-06, FR-07, FR-08 | FR-05, FR-09 |
| 품질 (QA) | QA-01, QA-02, QA-03, QA-04, QA-05, QA-07, QA-08, QA-11 | QA-06, QA-09, QA-10 |
...
flowchart TB
    ...
    subgraph fr_d["기능 Driver (구조에 배치)"]
        F12["FR-01·02<br/>생명주기·다중 pVM"]
        F34["FR-03·04<br/>HW IP 공유·보안 통신"]
        F67["FR-06·07<br/>동적 탑재·Secure OS 실행"]
        F8["FR-08<br/>TrustZone 연동"]
    end
...
|:----:|------|------------|----------|
| 1 | 신뢰 모델 확정 — TCB 범위(Host 측 프레임워크의 TCB 제외 여부), 공격자 모델, 보호 범위(기밀성·무결성 보장 / 가용성 제외 등) 결정 | QA-01, QA-02, CONST-02 | T-1~T-5 공통 전제 (특히 T-1의 책임 배치 결정) |
| 2 | 생명주기·다중 pVM 관리 골격 설계 | FR-01, FR-02, CONST-01 | T-1 |
| 3 | HW IP 중재 구조 실현 가능성 검증 (PoC) | FR-03, QA-03, QA-11, CONST-05 | T-2 |
| 4 | 통신/데이터 경로(zero-copy 보안 채널) 구조 설계 | FR-04, QA-04, QA-05 | T-3 |
| 5 | 확장 인터페이스 계약(패키지 형식·게스트 ABI) 고정 | FR-06, FR-07, QA-07, QA-08, CONST-06 | T-4 |
| 6 | TrustZone 연동 구조 배치 | FR-08, CONST-03 | T-5 |
...
| 4. Architectural Driver 선정 | `04_architectural_drivers.md` (본 문서) | Driver 21건 = 중요 FR 7 + 핵심 QA 8 + CONST 전체 6 |
