...
> 선행 DP는 4.3절의 DP 선행 의존성 다이어그램을 기준으로 한다. 표기 없는 DP는 해당 DP 완료 후 착수할 수 있고(실선 의존), `(병행)` 표기는 해당 DP와 병행 착수하거나 그 결과를 참조하는 약한 의존(점선 의존)을 의미한다.
...
DP-01과 DP-02는 후속 DP의 평가 기준이 되며, 이 DP들이 미결 상태이면 나머지 DP의 후보 구조를 일관되게 평가하기 어렵다.

| 우선순위 | DP | 이유 |
|:--------:|----|------|
| 1 | DP-01 (신뢰 모델) | 선행 DP가 없는 출발점. TCB 범위와 보호 속성이 미결이면 후속 DP 전체의 구조 선택지를 평가할 수 없음 |
| 1 | DP-02 (보안 정책·권한 집행 모델) | DP-04, DP-05의 공통 선행 DP. 정책 주체와 집행 위치가 미결이면 후속 DP들이 서로 다른 권한 가정으로 설계됨 |
| 2 | DP-03 (TrustZone 연동) | DP-07(키 관리), DP-09(부팅 검증)의 신뢰 앵커 선택에 선행함 |
| 2 | DP-05 (HW IP 중재) | 본 과제 최대 기술 리스크. DP-04의 최소 계약이 잡히는 즉시 병행 PoC로 실현 가능성을 조기 검증해야 전체 구조 방향을 확정할 수 있음 |
| 3 | DP-07, DP-09 | 각자의 선행 DP(DP-03, DP-06, DP-08) 설계와 맞물려 병행 검토 |
...
|:----:|---------|----------|
| 1 | DP-01 | 없음 |
| 2 | DP-02, DP-03 | DP-01 완료 |
| 3 | DP-04, DP-05 | DP-04: DP-02 완료. DP-05: DP-02 완료 후 DP-04의 최소 계약 확정 시 병행 착수 |
| 4 | DP-06 | DP-05 완료 |
| 5 | DP-07 | DP-03, DP-06 완료 |
| 6 | DP-08, DP-09 | DP-08: DP-07 완료. DP-09: DP-03 완료 후 DP-08과 병행 |

### 4.3 DP 선행 의존성 다이어그램

실선 화살표(`→`)는 선행 조건(해당 DP가 완료되어야 다음 DP를 착수할 수 있음), 점선 화살표(`-.->`)는 병행 착수 또는 결과를 참조하는 약한 의존성을 나타낸다. 빨간색 선은 critical path를 나타낸다.
...
flowchart TD
    ...
    linkStyle 0,2,3,4,5,7,8,10 stroke:#d32f2f,stroke-width:3px
```
