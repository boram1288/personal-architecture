...
> 선행 DP는 4절의 착수 시점 의존성 다이어그램을 기준으로 한다. 표기 없는 DP는 해당 DP 완료 후 착수할 수 있고(실선 의존), `(병행)` 표기는 해당 DP와 병행 착수하거나 그 결과를 참조하는 약한 의존(점선 의존)을 의미한다.
...
DP-01, DP-11, DP-07, DP-12는 **후속 DP의 평가 기준 또는 증빙 기준**이 되며, 이 DP들이 미결 상태이면 나머지 DP의 후보 구조를 일관되게 평가하기 어렵다.

| 우선순위 | DP | 이유 |
|:--------:|----|------|
| 1 | DP-01 (신뢰 모델) | TCB 범위와 보호 속성이 미결이면 DP-02~06 전체의 구조 선택지를 평가할 수 없음 |
| 1 | DP-12 (보안 정책·권한 집행 모델) | pVM 생성, HW IP 사용, 채널 수립, 워크로드 로딩 권한의 정책 주체와 집행 위치가 미결이면 DP-03~05·DP-10의 권한 경계가 흔들림 |
...
| 2 | DP-03 (HW IP 중재) | DP-02의 최소 pVM 생성·메모리 할당·hypercall 계약이 잡히면 병행 PoC로 실현 가능성을 먼저 검증해야 전체 구조 방향을 확정할 수 있음 |
...
# TODO
[] `fable 5` 모델로 `DP 착수 시점 의존성 다이어그램`을 다시 그려야 함.
